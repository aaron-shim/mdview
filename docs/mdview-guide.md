...
- 문단에 홀로 선 이미지(또는 이미지만 담은 HTML 블록)는 그림으로 그린다. sixel 터미널(DA1 응답에 4)이면
  페이저·미리보기에서 화면 픽셀 그대로(렌더러는 빈 줄로 자리만 잡고 `Placement`를 돌려주며, 출력기가
  그린 뒤 sixel을 얹는다. 이미지가 움직이면 화면을 지우고 다시 그린다), 아니면 반블록 `▀` + 트루컬러.
...
- `-a, --hidden`(별칭 `--all`): 브라우저를 숨김 파일·폴더까지 포함해 연다
...
- 숨김 파일·폴더(0.3.5): `.`로 켜고 끈다(`-a`로 켠 채 시작). `scan_markdown_files(root, hidden)`가
  `WalkBuilder::hidden(!hidden)`으로 훑고, `.git`은 `filter_entry`로 어느 쪽이든 건너뛴다.
  끌 때는 `is_hidden_path`로 목록·표식·변경 표시·접힘 상태에서 숨김 경로를 바로 걷어 내 곧장 반응하고,
  켤 때는 보던 목록을 둔 채 다시 훑은 결과를 기다린다. 다시 훑기는 `baseline`을 버려
  설정 때문에 늘거나 준 파일에 `● 새 파일`·`삭제됨`이 붙지 않게 한다.
  감시 스레드에는 `Request { root, hidden }`으로 주문하고, 스냅숏도 `hidden`을 달고 와서
  설정을 바꾸기 전에 출발한 결과는 버린다. 같은 루트에서 설정만 바뀐 경우에는 화면이 비지 않으므로
  부분 스냅숏을 흘려보내지 않는다. 헤더의 `·숨김`, 푸터의 `. hidden ✓`, 흐린 이름으로 상태를 보인다.
...
src/
  ...
  render/image.rs    이미지 읽기(경로 해석, 원격 내려받기, 디코딩 캐시) → 반블록 줄 또는 sixel 자리
  sixel.rs           sixel 지원 감지(DA1)·칸 픽셀 크기, median cut 색 줄이기 + 디더링, 인코딩
  output/graphics.rs 화면에 그린 sixel 추적, 스크롤·잘림 처리 (페이저·미리보기 공용)
