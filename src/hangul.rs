//! 한글 자소 결합(NFD → NFC).
//!
//! macOS는 파일 이름과 일부 편집기 출력에서 한글을 첫소리·가운뎃소리·끝소리로
//! 나눠 저장한다(이른바 자소 분리). 그대로 두면 `ㅎㅏㄴ` 처럼 보이고 표시 폭
//! 계산도 어긋나므로, 읽어 들일 때 미리 하나의 음절로 합친다.
//!
...
fn is_lv_syllable(c: u32) -> bool {
    (S_BASE..S_BASE + S_COUNT).contains(&c) && (c - S_BASE).is_multiple_of(T_COUNT)
}
